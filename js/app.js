...
(() => {
  ...
  const gradeScale = [
    ...
    {min:75, max:84.99, grade:"A", description:"Merit Pass"},
    {min:65, max:74.99, grade:"B+", description:"Very Good Pass"},
    {min:55, max:64.99, grade:"B", description:"Good Pass"},
    {min:45, max:54.99, grade:"C+", description:"General Pass"},
    {min:40, max:44.99, grade:"C", description:"Weak Pass"},
    {min:0, max:39.99, grade:"F", description:"Fail"}
  ];

  function gradeFor(value){
    const text = String(value ?? "").trim().toUpperCase();
    if (text === "N/A") return {grade:"N/A",description:"Not available"};
    const n = Number(value);
    if (!Number.isFinite(n)) return {grade:"—",description:"Not available"};
    // Grade bands use their lower boundary, so decimal averages such as 64.77
    // correctly receive B instead of falling through to F.
    return gradeScale.find(g => n >= g.min) || gradeScale.at(-1);
  }
  function esc(value){
  ...
  function renderStudentResult(student,batch){
    ...
    const moduleRows=(batch.modules||[]).map((m,i)=>{
      ...
      const isAB=markText==="AB";
      const isNA=markText==="N/A";
      const n=Number(mark);
      const cls=isNA?"mark-na":(isAB?"mark-ab":(Number.isFinite(n)&&n<40?"mark-fail":"mark-pass"));
      const grade=isNA?"N/A":(isAB?"Absent":(Number.isFinite(n)?esc(gradeFor(n).grade):"—"));
      return `<tr><td>${esc(m.code)}</td><td>${esc(m.name)}</td><td class="${cls}">${esc(mark)}</td><td>${grade}</td></tr>`;
    }).join("");
    const final = student.final ?? "—";
    const finalText=String(final ?? "").trim().toUpperCase();
    const finalGrade = gradeFor(final);
    const finalClass = finalText==="N/A"?"mark-na":(finalText==="AB"?"mark-ab":(Number.isFinite(Number(final)) && Number(final)<40 ? "mark-fail" : "mark-pass"));
  ...
  function renderStaffTable(){
    ...
    body.innerHTML=filtered.map((s,idx)=>{
      const marks=(s.marks||[]).map(m=>{
        const mt=String(m ?? "").trim().toUpperCase();
        const cls=mt==="N/A"?"mark-na":(mt==="AB"?"mark-ab":(Number.isFinite(Number(m))&&Number(m)<40?"mark-fail":""));
        return `<td class="${cls}">${esc(m)}</td>`;
      }).join("");
